...
Private Function CreateNewMailItem(objOlApp, objOlItem, mailAddress)
    ...
    Select Case mailAddress
        ...
        subject = "【テスト】【kintone保守 問合せ】" & objOlItem.Subject
...
Private Sub SaveTmpFiles(mailItem, path)
	...
	dim saveFileName
	saveFileName = attachmentFile 

	dim kinshiStrings
	kinshiStrings = "\,/,:,*,?,"",<,>,|"

	dim kinshiMoji
	For Each kinshiMoji In split(kinshiStrings ,",")
            saveFileName = replace(saveFileName ,kinshiMoji ,"_")
	next

        Call attachmentFile.SaveAsFile(path & "\" & saveFileName)
    Next
End Sub
